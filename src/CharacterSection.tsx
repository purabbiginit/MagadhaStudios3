...
const CharacterSection: React.FC = () => {
  ...
  return (
    <section className="relative w-full min-h-screen bg-black text-white xl:pt-24  overflow-hidden">

       {/* 🌿 Animated Left Grass */}
      <img
        src="/animatedleftgrass.svg"
        alt="Left Grass"
        className="md:top-[12%] top-[8%]"
        style={{
          position: "absolute",
          left: 0,
      ...
      {/* Title */}
      <h2  ref={headingRef}
      className="text-center font-instrument text-3xl md:text-7xl font-bold mb-12 
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-4 gap-x-4 gap-y-14 sm:gap-x-6 px-4 md:px-12 max-w-full mx-auto relative z-10">
        {characters.slice(0, visibleCount).map((char) => (
          ...
          >
            {/* 3D Gradient Glow */}
          <div
  className={`absolute -inset-2 rounded-2xl bg-gradient-to-tr ${char.glow}
  opacity-0 group-hover:opacity-80 blur-2xl transition duration-100
  group-hover:animate-glow bg-[length:200%_200%]`}
></div>
